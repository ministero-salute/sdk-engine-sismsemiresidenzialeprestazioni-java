...
package it.mds.sdk.flusso.sism.semiresidenziale.service;
...
import it.mds.sdk.flusso.sism.semiresidenziale.parser.regole.ParserTracciatoImpl;
...
@Slf4j
@Component("flussoSismSemiResService")
public class FlussoSismSemiresService {

    private static final String XML_FLUSSO_TAG = "</SemiResidenzialePrestazioniSanitarie><SemiResidenzialePrestazioniSanitarie>";
    ...
    private final String NOME_FILE_XSD_SISM = "PSS.xsd";
    ...
    private TracciatoSplitter<RecordDtoSismSemiresidenziale> tracciatoSplitter;
    private ConnettoreMds connettoreMds;
    private final ConfigurazioneFlussoSismSemires conf;

    @Autowired
    public FlussoSismSemiresService(@Qualifier("parserRegoleSismSemiRes") final ParserRegole parserRegole,
                                    @Qualifier("parserTracciatoSismSemiRes") final ParserTracciato parserTracciato,
                                    @Qualifier("validationEngine") final ValidationEngine validationEngine,
                                    @Qualifier("tracciatoSplitterSismSemiRes") final TracciatoSplitter<RecordDtoSismSemiresidenziale> tracciatoSplitter,
                                    @Qualifier("connettoreMds") final ConnettoreMds connettoreMds,
                                    @Qualifier("configurazioneFlussoSismSemiRes") ConfigurazioneFlussoSismSemires conf) {
    ...
    public void validazioneBlocchi(int dimensioneBlocco, String nomeFile, RegoleFlusso regoleFlusso, String idRun,
        ...
        SemiResidenzialePrestazioniSanitarie semiResidenzialePrestazioniSanitarie = null;
        File file = getfileFromPath(conf.getFlusso().getPercorso() + nomeFile);
        String nomeFileXml = conf.getXmlOutput().getPercorso() + "SDK_RES_PSS_" + periodoRiferimento + "_" + idRun + ".xml";
        ...
        do {
            ...
            List<RecordDtoSismSemiresidenziale> recordConv =
                    bloccoValidazione.getRecordList().stream().map(k -> (RecordDtoSismSemiresidenziale) k).collect(Collectors.toList());
            ...
            if (!recordConv.isEmpty()) {
                ...
                try {
                    semiResidenzialePrestazioniSanitarie = ((TracciatoSplitterImpl) tracciatoSplitter).creaSemiResidenzialePrestazioniSanitarie(recordConv, semiResidenzialePrestazioniSanitarie);
            ...
            if (isWriteOk) {
                ...
                URL urlXsd = this.getClass().getClassLoader().getResource("PSS.xsd");
                ...
                try {
                    gestoreFile.scriviDtoFragment(semiResidenzialePrestazioniSanitarie, nomeFileXmlTmp, urlXsd);
                ...
                semiResidenzialePrestazioniSanitarie = null;
                System.gc();
            }
